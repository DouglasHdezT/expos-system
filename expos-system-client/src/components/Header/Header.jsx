...
const Header = () => {
  ...
  return (
    <div className='p-4 sticky -top-4 left-0 z-[5]'>
        ...
        <div className="navbar-start">
          <div className="dropdown">
            ...
            <ul className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-300 rounded-box w-52">
              <li><Link to={"/evaluation"}> Evaluation criteria </Link></li>
              <li><Link to={"/stats"}> Stats </Link></li>
            </ul>
          </div>
        </div>
        <div className="navbar-center">
          <Link to={"/"} className="btn btn-ghost text-xl"> Web Expos 2023 </Link>
        </div>
        ...
        <label className="swap swap-rotate">
          ...
          <div className="dropdown dropdown-end">
            <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar">
                {
                  user ? <span className='text-primary text-xl'> { user.name.substring(0, 2).toUpperCase() } </span> : 
            ...
            <ul className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-300 rounded-box w-52">
              {
                user ? 
                  (
                    <>
                      <li>
                        <Link to="/own" className="justify-between">
                          { user.username }
                          <span className="badge">Subs</span>
                        </Link>
                      </li>
